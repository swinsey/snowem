...
{
  uint16_t testlen = stun_getw (msg->buffer + STUN_MESSAGE_LENGTH_POS) + STUN_MESSAGE_HEADER_LENGTH;
...
const void *
stun_message_find (const StunMessage *msg, int /*StunAttribute*/ type,
    uint16_t *palen)
{
  ...
  while (offset < length)
  {
    ...
    {
      *palen = alen;
      //ICE_HEXDUMP(msg->buffer + offset, alen, "value");
...
StunMessageReturn
stun_message_find_flag (const StunMessage *msg, int /*StunAttribute*/ type)
{
  const void *ptr;
  uint16_t len = 0;
...
StunMessageReturn
stun_message_find32 (const StunMessage *msg, int /*StunAttribute*/ type,
    uint32_t *pval)
{
  const void *ptr;
  uint16_t len = 0;
...
StunMessageReturn
stun_message_find64 (const StunMessage *msg, int /*StunAttribute*/ type,
    uint64_t *pval)
{
  const void *ptr;
  uint16_t len = 0;
...
StunMessageReturn
stun_message_find_string (const StunMessage *msg, int /*StunAttribute*/ type,
    char *buf, size_t buflen)
{
  const unsigned char *ptr;
  uint16_t len = 0;

  ptr = (const unsigned char*)stun_message_find (msg, type, &len);
  if (ptr == NULL)
    return STUN_MESSAGE_RETURN_NOT_FOUND;
...
StunMessageReturn
stun_message_find_addr (const StunMessage *msg, int /*StunAttribute*/ type,
...
StunMessageReturn
stun_message_find_xor_addr (const StunMessage *msg, int /*StunAttribute*/ type,
    struct sockaddr_storage *addr, socklen_t *addrlen)
{
...
StunMessageReturn
stun_message_find_xor_addr_full (const StunMessage *msg, int /*StunAttribute*/ type,
...
stun_message_find_error (const StunMessage *msg, int *code)
{
  ...
  class_ = ptr[2] & 0x7;
  number = ptr[3];
  if ((class_ < 3) || (class_ > 6) || (number > 99))
    return STUN_MESSAGE_RETURN_INVALID;

  *code = (class_ * 100) + number;
  return STUN_MESSAGE_RETURN_SUCCESS;
}
...
stun_message_append (StunMessage *msg, StunAttribute type, size_t length)
{
  ...
  uint16_t mlen = stun_getw (msg->buffer + STUN_MESSAGE_LENGTH_POS) + STUN_MESSAGE_HEADER_LENGTH;
    ...
    const struct sockaddr_storage *addr, socklen_t addrlen)
{
  ...
  val = stun_xor_address (msg, &tmpaddr, addrlen, STUN_MAGIC_COOKIE);
  STUN_DEBUG("get xor address, val=%u",val);
  if (val)
    return val;

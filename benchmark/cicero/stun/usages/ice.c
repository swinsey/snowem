    ...
    StunUsageIceCompatibility compatibility)
{
  ...
  if (compatibility == STUN_USAGE_ICE_COMPATIBILITY_WLM2009) {
    ...
    buf = (uint8_t*)malloc(attribute_len);
    ...
    StunUsageIceCompatibility compatibility)
{
  ...
  stun_debug ("Received %u-bytes STUN message", stun_message_length_new (msg));
...
#define err( code ) \
  ...
  *plen = 0;
  ...
  {
    STUN_DEBUG("STUN Role Conflict detected, control=%u",*control);
  ...
  if (compatibility == STUN_USAGE_ICE_COMPATIBILITY_MSN) {
    ...
    val = stun_message_append_xor_addr (msg, STUN_ATTRIBUTE_XOR_MAPPED_ADDRESS, src, srclen);
     STUN_DEBUG("get address, val=%u",val);
  } else {

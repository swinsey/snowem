...
void stun_agent_init (StunAgent *agent, const uint16_t *known_attributes,
    StunCompatibility compatibility, int usage_flags)
{
  int i;
    ...
    StunMessageIntegrityValidate validater, void * validater_data)
{
  ...
  ICE_DEBUG("STUN demux: stun_agent_validate, usage_flag=%x",agent->usage_flags);
  ...
  if ((agent->compatibility == STUN_COMPATIBILITY_RFC5389 ||
      ...
      ICE_DEBUG("STUN demux error: no cookie!");
  ...
  if ((agent->compatibility == STUN_COMPATIBILITY_RFC5389 ||
    ...
    if (stun_message_find32 (msg, STUN_ATTRIBUTE_FINGERPRINT, &fpr) !=
      ...
      ICE_DEBUG("STUN demux error: no FINGERPRINT attribute!");
      return STUN_VALIDATION_BAD_REQUEST;
    }

    /* Checks FINGERPRINT */
    crc32 = stun_fingerprint (msg->buffer, stun_message_length_new (msg),
        agent->compatibility == STUN_COMPATIBILITY_WLM2009);

    fpr = ntohl (fpr);
    if (fpr != crc32) {
      ICE_DEBUG("STUN demux error: bad fingerprint: 0x%08x,expected: 0x%08x!", fpr, crc32);
      return STUN_VALIDATION_BAD_REQUEST;
    }

    ICE_DEBUG("STUN demux: OK!");
  }
  ...
  ICE_DEBUG("ignore_credentials=%u",ignore_credentials);
      ...
      if (agent->usage_flags & STUN_AGENT_USAGE_LONG_TERM_CREDENTIALS) {
        ...
        if (agent->compatibility == STUN_COMPATIBILITY_RFC3489 ||
          ...
          stun_sha1 (msg->buffer, hash + 20 - msg->buffer,
              stun_message_length_new (msg) - 20, sha, md5, sizeof(md5), TRUE);
        } else {
          stun_sha1 (msg->buffer, hash + 20 - msg->buffer,
        ...
        if (agent->compatibility == STUN_COMPATIBILITY_RFC3489 ||
          ...
          stun_sha1 (msg->buffer, hash + 20 - msg->buffer,
              stun_message_length_new (msg) - 20, sha, key, key_len, TRUE);
        } else {
          stun_sha1 (msg->buffer, hash + 20 - msg->buffer,
    ...
    const uint8_t *key, size_t key_len)
{
  ...
  if (key != NULL) {
    ...
    if (skip == FALSE) {
      ptr = (uint8_t*)stun_message_append (msg, STUN_ATTRIBUTE_MESSAGE_INTEGRITY, 20);
      if (ptr == NULL) {
        return 0;
      }
      if (agent->usage_flags & STUN_AGENT_USAGE_LONG_TERM_CREDENTIALS) {
        if (agent->compatibility == STUN_COMPATIBILITY_RFC3489 ||
            agent->compatibility == STUN_COMPATIBILITY_OC2007) {
          stun_sha1 (msg->buffer, stun_message_length_new (msg),
              stun_message_length_new (msg) - 20, ptr, md5, sizeof(md5), TRUE);
        } else if (agent->compatibility == STUN_COMPATIBILITY_WLM2009) {
          size_t minus = 20;
          ...
          stun_sha1 (msg->buffer, stun_message_length_new (msg),
              stun_message_length_new (msg) - minus, ptr, md5, sizeof(md5), TRUE);
        } else {
          stun_sha1 (msg->buffer, stun_message_length_new (msg),
              stun_message_length_new (msg) - 20, ptr, md5, sizeof(md5), FALSE);
        }
      } else {
        if (agent->compatibility == STUN_COMPATIBILITY_RFC3489 ||
            agent->compatibility == STUN_COMPATIBILITY_OC2007) {
          stun_sha1 (msg->buffer, stun_message_length_new (msg),
              stun_message_length_new (msg) - 20, ptr, key, key_len, TRUE);
        } else if (agent->compatibility == STUN_COMPATIBILITY_WLM2009) {
          size_t minus = 20;
          if (agent->usage_flags & STUN_AGENT_USAGE_USE_FINGERPRINT)
            minus -= 8;
          ...
          stun_sha1 (msg->buffer, stun_message_length_new (msg),
              stun_message_length_new (msg) - minus, ptr, key, key_len, TRUE);
        } else {
          stun_sha1 (msg->buffer, stun_message_length_new (msg),
              stun_message_length_new (msg) - 20, ptr, key, key_len, FALSE);
        }
      }
  ...
  if ((agent->compatibility == STUN_COMPATIBILITY_RFC5389 ||
          agent->compatibility == STUN_COMPATIBILITY_WLM2009) &&
      agent->usage_flags & STUN_AGENT_USAGE_USE_FINGERPRINT) {
    ptr = (uint8_t*)stun_message_append (msg, STUN_ATTRIBUTE_FINGERPRINT, 4);
    if (ptr == NULL) {
      return 0;
    }

    fpr = stun_fingerprint (msg->buffer, stun_message_length_new (msg),
        agent->compatibility == STUN_COMPATIBILITY_WLM2009);
  ...
  msg->key = (uint8_t *) key;
  msg->key_len = key_len;
  return stun_message_length_new (msg);

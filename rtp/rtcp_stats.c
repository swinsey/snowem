...
snw_rtcp_stats_find(snw_rtp_ctx_t *ctx, uint32_t ssrc) {
   ...
   list_for_each(n,&ctx->receiver_stats.list) {
      snw_rtcp_stats_t* s = list_entry(n,snw_rtcp_stats_t,list);
      if (s->ssrc == ssrc) {
         DEBUG(log,"found receiver stats, ssrc=%u", s->ssrc);
         stats = s;
         break;
      }
   }
...
snw_rtcp_stats_new(snw_rtp_ctx_t *ctx, uint32_t ssrc) {
   ...
   stats->rtcp_interval = RTCP_MIN_INTERVAL;
   list_add(&stats->list,&ctx->receiver_stats.list);
...
snw_rtp_slidewin_reset(snw_rtp_ctx_t *ctx, rtp_slidewin_t *win, uint16_t seq) {
   ...
   DEBUG(log,"reset window, seq=%u", seq);
   return;
}
...
snw_rtp_slidewin_update(snw_rtp_ctx_t *ctx, rtp_slidewin_t *win, uint16_t seq, uint16_t end) {
   ...
   while(seq != end) {
      if (win->seqlist[i].seq != seq) {
         DEBUG(log, "lost seq, seq=%u, end=%u", seq, end);
...
snw_rtp_slidewin_put(snw_rtp_ctx_t *ctx, rtp_slidewin_t *win, uint16_t seq) {
   ...
   DEBUG(log, "slidewin put, i=%u, udelta=%u, seq=%u, last_seq=%u, head=%u",
   ...
   if (udelta < RTP_SLIDEWIN_SIZE) {
      ...
      DEBUG(log, "reordered pkt, udelta=%u, seq=%u", udelta, seq); 
      idx = seq % RTP_SLIDEWIN_SIZE;
      win->seqlist[idx].seq = seq;
      win->seqlist[idx].status = RTP_RECV;
   }
